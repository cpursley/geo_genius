defmodule GeoGenius do
  ...
  alias GeoGenius.{Catalog, CatalogError, ImportRun, Manifest, ManifestError}
  ...
  defp start_import(context, %Manifest{} = manifest, opts) do
    ...
    release_id = register_manifest(context, manifest)
  ...
  # Steps 2 through 6: registering a manifest is idempotent end to end, so a
  # release re-imported under the same key re-registers the same rows.
  defp register_manifest(context, %Manifest{} = manifest) do
